    ...
    that.filterArticlesNameFromJSONString = function(a_options){
        ...
        magazineJSON.sections.forEach(function(a_section, a_index) {
            ...
            a_section.articles.forEach(function(a_article, a_index) {
                arr.push({title:a_article.title,id:a_article.id,localName:a_article.title+'['+a_article.id+']'});
            });
        });
        ...
        if(!a_options.config.articles){
            a_options.articles.forEach(function(a_article, a_index) {
                ...
                try{
                    if(!fs.existsSync(a_article.localName)){
                        console.log('first update, fs.mkdir,title is:',a_article.localName)
                        fs.mkdir(a_article.localName);
                    }else{
                        console.warn('find same name:',CMSArticlesName[a_articleSaved.id].localName)
                    }
                }catch(e){
            ...
            a_options.config.articles.forEach(function(a_articleSaved, a_articleSavedIndex) {
                ...
                if(!CMSArticlesName[a_articleSaved.id]){
                    try{
                        if(!fs.existsSync(CMSArticlesName[a_articleSaved.id].localName)){
                            console.log('fs.mkdir, title is:',CMSArticlesName[a_articleSaved.id].localName)
                            fs.mkdir(CMSArticlesName[a_articleSaved.id].localName);
                        }else{
                            console.warn('find same name:',CMSArticlesName[a_articleSaved.id].localName)
                        }
                    }catch(e){
                        console.error('creatFolderByArticlesName, fs.mkdir error:',e)
                    }
                // local ID exist,and title changed, rename item
                }else if(CMSArticlesName[a_articleSaved.id].localName != a_articleSaved.localName){
                    if(fs.existsSync(a_articleSaved.localName)){
                        fs.rename(a_articleSaved.localName, CMSArticlesName[a_articleSaved.id].localName);
                    }else{
                        console.warn('title changed, and rename a_articleSaved.localName error,a_articleSaved.localName is:',a_articleSaved.localName);
                    }
                }
            })
            for(var i in LocalArticlesName){
                ...
                if(!CMSArticlesName[i]){
                    if(fs.existsSync(LocalArticlesName[i].localName)){
                        fs.rename(LocalArticlesName[i].localName,'[del]'+LocalArticlesName[i].localName);
                    }else{
