...
function UploadToCMS(){
    ...
    that.init = function(options) {
        ...
        loadDB(options.path,function(a_json) {
            ...
            that.scanHTMLZip({config:a_json,callback:function(data,array) {
    ...
    that.scanHTMLZip = function(a_options) {
        ...
        if(a_options.config.articles){
                ...
                if(a_article.html){
                    for(var a_HTML in a_article.html){
                        if('zip' == a_article.html[a_HTML].type){
                            fileString = fs.readFileSync(a_article.title+'/'+a_HTML+'.zip');
                            waiting4upload.push({
                                page:a_HTML,
                                articleIndex : a_index,
                                articleID : a_article.id,
                                zipPath:a_article.title+'/'+a_HTML+'.zip',
            ...
            console.log('scanHTMLZip, waiting4upload is:',waiting4upload)
    ...
    that.checkHTMLInCMS = function(a_options){
        ...
        var req = http.request(options, function(res) {
            ...
            console.error('checkHTMLInCMS error: ', e);
        });;
        req.write("");
        req.end();
            ...
            for (var i = $materialForm.length - 1; i >= 0; i--) {
                if(0==$($materialForm[i]).find('.material-edit')){// 未上传的
                    ...
                    if(a_options.zipConfig.resID || a_options.zipConfig.resID == o.resID){
                        console.log('find one in CMS, resID is:',o.resID);
                        return o;
                    }
    ...
    that.uploadCode = function(a_options, a_callback){
        ...
        try{
            ...
            var zip_data = JSON.stringify({
                ...
                'comment': 'p'+(1+a_options.htmlConfig.page)+',resID is:'+a_options.htmlConfig.resID+', update by '+a_options.config.htmlauthor+ ' at :'+new Date(),
